...
| `get_available_basis_gates_tool` | Get preset basis gate sets |
| `get_available_topologies_tool` | Get available coupling map topologies |
| `get_transpiler_info_tool` | Get transpiler documentation |
...
**Parameters:**
- `circuit_qasm`: OpenQASM 2.0 or 3.0 string
- `optimization_level`: 0-3 (default: 2)
  - 0: No optimization, only basis gate decomposition
...
- `initial_layout`: List of physical qubit indices
...
|--------|-------|-------------|
| `ibm_default` | id, rz, sx, x, cx | Default IBM Quantum gates |
| `ibm_eagle` | id, rz, sx, x, cx, reset | IBM Eagle processor |
| `ibm_heron` | id, rz, sx, x, cz, reset | IBM Heron processor (uses CZ) |
| `generic_clifford_t` | h, s, t, cx | Universal Clifford+T |
| `ion_trap` | rx, ry, rz, rxx | Ion trap gates |
| `superconducting` | id, rz, sx, x, cx | Generic superconducting |
